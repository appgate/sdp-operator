...
dump-crd: docker-run-image
	docker run -v ${PWD}:/build --rm -it --entrypoint bash localhost:5000/sdp-operator ./run.sh --spec-directory /appgate/api_specs/$(VERSION) dump-crd --file /build/k8s/crd/templates/$(VERSION).yaml
