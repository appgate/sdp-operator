...
note that the operator listen events inside a namespace.
...
that no more events were received in that specified time) the operator will
proceed to compute and apply a new `Plan`.
...
will remove all the entities that are not part of the set of builting tags (see
...
*** Secrets
The operator supports right now 3 ways of dealing with secrets:
 - secrets unencrypted.
...
**** Encrypted secrets
We can also save an encrypted password in the yaml file defining the entity, in
...
**** k8s secret store
We can also use k8s secrets store mechanism to save the secrets and reference
...
in base64. For now we only support the enoded value directly in the yaml file.

** Building
We build one image per API version. Each image uses that specific API version by
default but contains the specs for all the API versions supported.
...
Before deploying it MUST define these env variables:
 ...
 - APPGATE_OPERATOR_FERNET_KEY :: FERNET key used to encrypt passwords when this
...
To deploy the oeprator inside the k8s cluster we need to follow almost the same
